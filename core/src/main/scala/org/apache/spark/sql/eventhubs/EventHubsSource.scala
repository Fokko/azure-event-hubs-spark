...
private[spark] class EventHubsSource private[eventhubs](sqlContext: SQLContext,
  ...
  private lazy val ehClient: Client = clientFactory(ehConf)
  ...
  override def getOffset: Option[Offset] = {
    ...
    val earliestSeqNos = Some(earliestAndLatest.map {
  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    // numbers are greater than or equal to the earliestSeqNos.
    // If not, we'll report possible data loss.
    val earliestSeqNos = earliestAndLatest.map {
      case (p, (e, _)) => NameAndPartition(ehName, p) -> e
    }.toMap
    ...
    logDebug(s"Partitions: ${nameAndPartitions.mkString(", ")}")
    val sortedExecutors = getSortedExecutorList(sc)
    val numExecutors = sortedExecutors.length
    logDebug(s"Sorted executors: ${sortedExecutors.mkString(", ")}")
